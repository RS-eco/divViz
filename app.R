...
server <- function(input, output){
  ...
  output$cat_choice <- renderUI({
    selectInput(inputId="spec", label="Wähle eine Art:", 
                choices = c("Alle Arten", sort(unique(datclass() %>% dplyr::select(art) %>% unlist()))),
                selected="Alle Arten")
  })
  
  output$cat_choice2 <- renderUI({
    selectInput(inputId="spec2", label="Wähle die erste Art:", 
                choices = sort(unique(datclass() %>% dplyr::select(art) %>% unlist())))
  })
  
  output$cat_choice3 <- renderUI({
    selectInput(inputId="spec3", label="Wähle die zweite Art:", 
                choices = sort(unique(datclass() %>% dplyr::select(art) %>% unlist())[unique(datclass() %>% 
                                                                                               dplyr::select(art) %>% unlist()) != input$spec2]))
  })
  
  datclassdist <- reactive({datclass() %>% filter(district %in% input$district)})
  
  dataspec <- reactive({
    if(input$spec != "Alle Arten"){
      datclassdist() %>% filter(art == input$spec)
    } else{
      datclassdist()
    }
  })
  ...
  dataspeccomp <- reactive({
    if(input$res == "TK25"){
      datclassdist() %>% filter(art %in% c(input$spec2, input$spec3)) %>% 
        group_by(XLU, XRU, YLU, YLO, art, class_order) %>% 
        summarise(`Species richness`=n_distinct(art, na.rm=T), 
                  `Number of records`=n()) %>% 
        filter(`Number of records` >= input$sp_weight)
    } else {
      datclassdist() %>% filter(art %in% c(input$spec2, input$spec3)) %>% 
        group_by(XLU_rough, XRU_rough, YLU_rough, YLO_rough, art, class_order) %>% 
        summarise(`Species richness`=n_distinct(art, na.rm=T), 
                  `Number of records`=n()) %>% 
        rename(XLU=XLU_rough, XRU=XRU_rough, YLU=YLU_rough, YLO=YLO_rough) %>%
        filter(`Number of records` >= input$sp_weight)
  ...
  datatime <- reactive({
    dat <- dataspec()
    if(input$res == "TK25"){
      dat %>% group_by(jahr, class_order) %>% 
        summarise(`Species richness`= n_distinct(art, na.rm=T), 
                  `Number of records`= n(),
                  `Number of occupied grid cells`= n_distinct(XLU, XRU, YLU, YLO, na.rm=T)) %>% 
        filter(`Number of records` >= input$temp_weight)
    } else {
      dat %>% group_by(jahr, class_order) %>% 
        summarise(`Species richness`= n_distinct(art, na.rm=T), 
                  `Number of records`= n(),
                  `Number of occupied grid cells`= n_distinct(XLU_rough, XRU_rough, YLU_rough, YLO_rough, na.rm=T)) %>% 
        filter(`Number of records` >= input$temp_weight)
    }
  })
  ...
  dataspacetime <- reactive({
    dat <- dataspec()
    if(input$res == "TK25"){
      dat %>% group_by(jahr, XLU, XRU, YLU, YLO, karte, class_order) %>% 
        summarise(`Species richness`=n_distinct(art, na.rm=T), 
                  `Number of records`=n()) %>% 
        filter(`Number of records` >= input$sp_weight)
    } else {
      datclassdist() %>% ungroup() %>% 
  ...
  output$plot456 <- renderPlot({
    ...
    if(length(unique(sub_dat3$district)) >= 3){
      ...
      p6 <- sub_dat2 %>% ggplot() + geom_bar(aes(x=district, y=val, fill=class_order), stat="identity") + 
        scale_fill_manual(values=c("Vögel" = '#1b9e77', "Schmetterlinge"='#d95f02',
                                   "Libellen"='#7570b3', "Heuschrecken"='#e7298a')) + 
  ...
  output$plot11 <- renderPlot({
    dataspeccomp() %>% ggplot() + geom_rect(aes_string(xmin="XLU", xmax="XRU", ymin="YLU", ymax="YLO", fill="class_order")) + 
      facet_grid(.~art) + geom_sf(data=shape(), fill="transparent", col="black") +
      scale_fill_manual(values=c("Vögel" = '#1b9e77', "Schmetterlinge"='#d95f02',
                                 "Libellen"='#7570b3', "Heuschrecken"='#e7298a')) + 
      labs(x="Breitengrad", y="Längengrad") + 
      coord_sf(xlim = c(min(st_coordinates(shape())[,'X']),
  ...
  output$plot12 <- renderPlot({
    dataspeccomp() %>% ggplot() + 
      geom_rect(aes_string(xmin="XLU", xmax="XRU", ymin="YLU", 
                           ymax="YLO", fill="`Number of records`")) + facet_grid(.~art) + 
      scico::scale_fill_scico(name="Anzahl an\nBeobachtungen", palette="roma", na.value= "grey50", direction=-1) + 
      geom_sf(data=shape(), fill="transparent", col="black") +
      labs(x="Breitengrad", y="Längengrad") + 
      coord_sf(xlim = c(min(st_coordinates(shape())[,'X']),
